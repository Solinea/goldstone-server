...
var ApiBrowserView = ChartSet.extend({
    ...
    instanceSpecificInit: function() {
        ...
        if (this.stackOrTotal === 'stack') {
            this.makeChartAsStack();
            return;
        }
    ...
    makeChartAsStack: function() {
        ...
        this.x.domain(self.data.length ? self.data[0].map(function(d) {
            return d.x;
        }) : [1, 1])
            .range([0, (self.width - self.marginLeft - self.marginRight)]);
    ...
    updateAsStack: function() {
        var self = this;
        this.x.domain(d3.extent(self.data[0].map(function(d) {
            return d.x;
        })));

        this.y.domain([0, d3.max(self.data[self.data.length - 1], function(d) {
            return d.y0 + d.y;
        })]);
    ...
    update: function() {
        ...
        if (this.stackOrTotal === 'stack') {
            this.setData(this.collection.toJSON()[0].stacked);
            this.updateAsStack();
            return;
        } else {
            this.setData(this.collection.toJSON()[0].finalResult);
        }
