...
var ServiceStatusView = Backbone.View.extend({
    ...
    update: function() {
        ...
        _.each(nodeNames, function(item, i) {
            $(this.defaults.location).find('.mainContainer').append('<div style="font-size:11px; margin-bottom: -4px; padding: 3px 0;" class="col-xs-1 toRemove ' + classSelector(_.values(nodeNames[i])) + '">' + _.keys(nodeNames[i]) + '</div>');
        }, this);
