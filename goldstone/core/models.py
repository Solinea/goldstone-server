...
class PycadfEventConverter(NewDailyIndexDocType):

    """
        Convert a pycadf event into an internal system generated event
        via the DailIndexDocType class
    """

    # This is the index prefix with which we save and search for
    # events generated by the EventQueryDef class
    INDEX_PREFIX = 'eventquerydef-'
    SORT = '-@timestamp'

    class Meta:          # pylint: disable=C0111,W0232,C1001
        doc_type = 'core_eventquerydef'
        index = 'events_*'

    @classmethod
    def save(self, using=None, index=None, **kwargs):

        return super(PycadfEventConverter, self).save(using, index, **kwargs)
...
class SavedSearch(models.Model):
    ...
    def execute_query(cls, query_name):
        ...
        query = Q(QueryString(query=SavedSearch.objects.get(name=query_name)))
        return NewDailyIndexDocType.search().query(query)
...
class EventQueryDef(SavedSearch):
    ...
    def search_by_syslog_err_type(cls):
        ...
        # print result
        # result is an infinite list of dictionaries. why ?

        event_id_status_list = []
        for i in range(0, result[1].to_dict().get('size')):
